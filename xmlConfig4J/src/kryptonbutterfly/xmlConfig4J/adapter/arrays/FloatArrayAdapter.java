...
public final class FloatArrayAdapter implements TypeAdapter<float[]>
{
	...
	public float[] read(CommentReader cr, XmlReader reader, Node node, Class<?> classOfT)
	{
		...
		final var	result	= new float[(int) nodes.stream().filter(n -> isItemNode(reader, n)).count()];
		...
		for (final var n : nodes)
			if (n instanceof org.w3c.dom.Comment comment)
				...
				{
					result[index++] = FloatAdapter.read(reader.getTags().valueTag(), n);
				}
			else
			{
