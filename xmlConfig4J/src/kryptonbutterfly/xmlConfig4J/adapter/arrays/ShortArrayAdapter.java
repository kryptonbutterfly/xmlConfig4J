...
public final class ShortArrayAdapter implements TypeAdapter<short[]>
{
	...
	public short[] read(CommentReader cr, XmlReader reader, Node node, Class<?> classOfT)
	{
		...
		final var	result	= new short[(int) nodes.stream().filter(n -> isItemNode(reader, n)).count()];
		...
		for (final var n : nodes)
			if (n instanceof org.w3c.dom.Comment comment)
				...
				{
					result[index++] = ShortAdapter.read(reader.getTags().valueTag(), n);
				}
			else
			{
