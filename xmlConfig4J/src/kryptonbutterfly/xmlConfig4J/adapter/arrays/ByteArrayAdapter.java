...
public final class ByteArrayAdapter implements TypeAdapter<byte[]>
{
	...
	@Override
	public void write(CommentWriter cw, XmlWriter writer, Element elem, byte[] value)
	{
		if (value == null)
	...
	@Override
	public byte[] read(CommentReader cr, XmlReader reader, Node node, Class<?> classOfT)
	{
		...
		final var result = new byte[(int) nodes.stream().filter(n -> isItemNode(reader, n)).count()];
		...
		for (final var n : nodes)
			if (n instanceof org.w3c.dom.Comment comment)
				...
				{
					result[index++] = ByteAdapter.read(reader.getTags().valueTag(), n);
				}
			else
			{
