...
public final class LongArrayAdapter implements TypeAdapter<long[]>
{
	...
	@Override
	public void write(CommentWriter cw, XmlWriter writer, Element elem, long[] value)
	{
		if (value == null)
	...
	@Override
	public long[] read(CommentReader cr, XmlReader reader, Node node, Class<?> classOfT)
	{
		if (reader.isNull(node))
			return null;
		
		final var	nodes	= new Nodes(node.getChildNodes());
		final var	result	= new long[(int) nodes.stream().filter(n -> isItemNode(reader, n)).count()];
		...
		for (final var n : nodes)
			if (n instanceof org.w3c.dom.Comment comment)
				...
				{
					result[index++] = LongAdapter.read(reader.getTags().valueTag(), n);
				}
			else
			{
